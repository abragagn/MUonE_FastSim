...
//typedef ROOT::TMatrixD TMatrixD;
  ...
  private:
    ...
    void SmearSi (PxPyPzEVector & k, TMatrixD & c, const Double_t & nSi, const Double_t & before, const Double_t & id );
    void SmearBe (PxPyPzEVector & k, TMatrixD & c, const Double_t & f, const Double_t & before, const Double_t & id );
    void BlankSpace(PxPyPzEVector & k, TMatrixD & c, const Double_t & d, const Double_t & before, const Double_t & id );
    void Propagate (PxPyPzEVector & mu_in, PxPyPzEVector & mu_out,PxPyPzEVector & e_out, std::vector<PxPyPzEVector> & photons, TMatrixD & c, TMatrixD & def_angle,TMatrixD & def_angle_smear);
    void PrepareEMShower(KineVars & kv,PxPyPzEVector & e_out_mod, std::vector<PxPyPzEVector> & photons, TMatrixD & c, const Double_t & i,const Double_t & r_mu,GammaFunctionGenerator* & gamma, EMECALShowerParametrization* const & myParam, ECAL* const & myGrid);
    void LoadECAL (KineVars & kv, ECAL* const & myGrid,int j);

    TMatrixD Def_angle( PxPyPzEVector & p_mu_in, PxPyPzEVector & p_mu_out, PxPyPzEVector & p_e_out);
    Double_t Def_angle_ph(const PxPyPzEVector & p_mu_in,const PxPyPzEVector & p_gamma_lab) const;

    KineVars LoadKineVars(const PxPyPzEVector & p_mu_in,  const PxPyPzEVector & p_e_in,
          const PxPyPzEVector & p_mu_out, const PxPyPzEVector & p_e_out,TMatrixD & c,TMatrixD & def_angle,TMatrixD & def_angle_smear,const Double_t & tar,const Double_t & n_phot,ECAL* const & myGrid);
    void LoadPhotons(const std::vector<PxPyPzEVector> & photons, TMatrixD & coo,Double_t n_photons,const PxPyPzEVector & p_mu_in,ECAL* const & myGrid);
    MuonOut DetMuonOut(const PxPyPzEVector & p_mu_in, const PxPyPzEVector & p_mu_out,TMatrixD & c,TMatrixD & def_angle,TMatrixD & def_angle_smear,const Double_t & tar,const Double_t & n_phot) const;
    Electron DetElectron(const PxPyPzEVector & p_e_in, const PxPyPzEVector & p_e_out,TMatrixD & c,TMatrixD & def_angle,TMatrixD & def_angle_smear,ECAL* const & myGrid) const;
