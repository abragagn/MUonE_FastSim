...
void Analysis::Analyze(const MuE::Event & event, const MuE::FastSim & fs)
{
  ...
  const MuE::Photon & photon1  = fs.GetPhoton1();
  const MuE::Photon & photon2  = fs.GetPhoton2();
  ...
  myAna.genKin    = genKin;
  myAna.detKin    = detKin;
  myAna.photon1   = photon1;
  myAna.photon2   = photon2;
...
void Analysis::EndJob(const MCstat & mcsums, const FastSim & fs,
                      std::vector<std::pair<std::string,Long_t> > *mc_inputs)
{
  ...
  if (n_events > 0)
  {
    ...
    cerr << endl << "Processed " << n_events
         << " events, histograms written to file: results.root" << endl;
  }
  else
  {
    // load histos from existing external file
    auto fp = new TFile(parmain.histo_ifname.c_str());

    //    output_hist_file->cd();

    auto projfile = new TFile("projstat.root", "RECREATE");
    projfile->cd();
