...
Histos::Histos(const MuE::MCpara & pargen, const AN_Input & paran,
               const TString & nameDir, const TString & namePrefix,
               const bool make_dirs, bool debug) :
  Emin_e(pargen.Emin_e), charge_mu(pargen.charge_mu), mass_mu(pargen.mass_mu),
  mass_e(pargen.mass_e), invalfa0(pargen.invalfa0), UNWGT(pargen.UNWGT),
  Wnorm(pargen.Wnorm),  angleCorrPlots_finebins(paran.angleCorrPlots_finebins),
  doTemplates(paran.doTemplates), thetaMax(paran.thetaMax),
  theMaxTemp(paran.theMaxTemp), iLumi(paran.iLumi), rangeSigma(paran.rangeSigma),
  divSigma(paran.divSigma), iparam(paran.iparam),
  doEnergyScale(paran.doEnergyScale), dirname(nameDir), prefix(namePrefix),
  makedirs(make_dirs)
{
  ...
  if (debug)
  {
    cerr << "\n Testing " << kine_distributions.size()
         << " kinematical distributions in tuple:" << endl;
    for (const auto &im : kine_distributions)
      cerr << std::get<0>(im) << " " <<std::get<1>(im) << " " << std::get<2>(im)  << endl;
  }
  ...
  hn_t13 = new TH1D(prefix + "hn_t13",
                    "t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_t24 = new TH1D(prefix + "hn_t24",
                    "t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_Ee = new TH1D(prefix + "hn_Ee",
                    "Electron energy; E_{e} (GeV); N",
                    100, Emin_e, 140.);

  // max electron theta: for Emin_e=0.2 GeV set to 71 mrad; for Emin_e=5 GeV to 14 mrad
  Double_t themax = Emin_e < 5.  ?  71.  :  14. ;

  hn_the = new TH1D(prefix + "hn_the",
                  "Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hn_Emu = new TH1D(prefix + "hn_Emu",
                    "Muon energy; E_{#mu} (GeV); N",
                    100, 0., 150.);

  hn_thmu = new TH1D(prefix + "hn_thmu",
                  "Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hn_x13 = new TH1D(prefix + "hn_x13",
                    "x_13 (muon leg); x_{13}; N",
                    100, 0., 1.);

  hn_x24 = new TH1D(prefix + "hn_x24",
                    "x_24 (electron leg); x_{24}; N",
                    100, 0., 1.);

  //  hn_deltaPhi = new TH1D(prefix+"hn_deltaPhi","delta(Phi) = phi(e) - phi(mu); #Delta#phi (rad); N",100,-TMath::Pi(),+TMath::Pi());
  hn_deltaPhi = new TH1D(prefix + "hn_deltaPhi",
                    "delta(Phi) = phi(e) - phi(mu); #Delta#phi (rad); N",
                    100, -0.5, 0.5);

  if (angleCorrPlots_finebins)
  {
    hn_thmu_vs_the = new TH2D(prefix + "hn_thmu_vs_the",
                      "#theta_{#mu} vs #theta_{e}; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                      1000, 0., 30., 200, 0., 5.);
  }

  hn_openingAngle = new TH1D(prefix + "hn_openingAngle",
                    "mu-e opening angle; opening angle (mrad); N",
                    142, 0., 71.);

  hn_tripleProduct = new TH1D(prefix + "hn_tripleProduct",
                    "triple product (acoplanarity); triple product; N",
                    200, -1., +1.);

  hn_egamma = new TH1D(prefix + "hn_egamma",
                    "Photon energy; E_{#gamma} (GeV); N",
                    100, 0.75, 150.);

  hn_thgamma = new TH1D(prefix + "hn_thgamma",
                  "Photon angle; #theta_{#gamma} (mrad); N",
                  100, 0., 40.); // in mrad

  hn_egamma_CoM = new TH1D(prefix + "hn_egamma_CoM",
                    "Photon angle CoM frame; E_{#gamma} (GeV); N",
                    100, 0., 0.2);
  //
  //////////////////////////////////////////////////////////////////////////////
  ...
  hsn_t13 = new TH1D(prefix + "hsn_t13",
                    "DET t_13 = (pmu_in - pmu_out)^{2}; DET t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_t24 = new TH1D(prefix + "hsn_t24",
                    "DET t_24 = (pe_in - pe_out)^{2}; DET t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_Ee = new TH1D(prefix + "hsn_Ee",
                    "DET Electron energy; DET E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hsn_the = new TH1D(prefix + "hsn_the",
                  "DET Electron angle; DET #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsn_Emu = new TH1D(prefix + "hsn_Emu",
                    "DET Muon energy; DET E_{#mu} (GeV); N",
                    100, 0., 150.);

  hsn_thmu = new TH1D(prefix + "hsn_thmu",
                  "DET Muon angle; DET #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hsn_x13 = new TH1D(prefix + "hsn_x13",
                    "DET x_13 (muon leg); DET x_{13}; N",
                    100, 0., 1.);

  hsn_x24 = new TH1D(prefix + "hsn_x24",
                    "DET x_24 (electron leg); DET x_{24}; N",
                    100, 0., 1.);

  //  hsn_deltaPhi = new TH1D(prefix+"hsn_deltaPhi","DET delta(Phi) = phi(e) - phi(mu); DET #Delta#phi (rad); N",100,-TMath::Pi(),+TMath::Pi());
  hsn_deltaPhi = new TH1D(prefix + "hsn_deltaPhi",
                    "DET delta(Phi) = phi(e) - phi(mu); DET #Delta#phi (rad); N",
                    100, -0.5, 0.5);

  if (angleCorrPlots_finebins)
  {
    hsn_thmu_vs_the = new TH2D(prefix + "hsn_thmu_vs_the",
                      "DET #theta_{#mu} vs #theta_{e}; DET #theta_{e} (mrad); DET #theta_{#mu} (mrad)",
                      1000, 0., 30., 200, 0., 5.);
  }

  hsn_openingAngle = new TH1D(prefix + "hsn_openingAngle",
                    "DET mu-e opening angle; DET opening angle (mrad); N",
                    142, 0., 71.);

  hsn_tripleProduct = new TH1D(prefix + "hsn_tripleProduct",
                    "DET triple product (acoplanarity); DET triple product; N",
                    200, -1., +1.);

  hsn_coo_mu = new TH2D(prefix + "hsn_coo_mu",
                    "DET coordinates mu; DET #x_{mu} (m); DET #y_{#mu} (m)",
                    200, -0.2, 0.2, 200, -0.2, 0.2);

  hsn_coo_e = new TH2D(prefix + "hsn_coo_e",
                    "DET coordinates e; DET #x_{e} (mrad); DET #y_{#e} (mrad)",
                    200, -0.2, 0.2, 200, -0.2, 0.2);

  hsn_thmu_vs_the_defAngles = new TH2D(prefix + "hsn_thmu_vs_the_defAngles",
                    "DET #theta_{#mu} vs #theta_{e}; DET #theta_{e} (mrad); DET #theta_{#mu} (mrad)",
                    180, 0., 30., 250, 0., 5.);
  //////////////////////////////////////////////////////////////////////////////
  ...
  hn_t13_norun = new TH1D(prefix + "hn_t13_norun",
                    "t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_t24_norun = new TH1D(prefix + "hn_t24_norun",
                    "t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_Ee_norun = new TH1D(prefix + "hn_Ee_norun",
                    "Electron energy; E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hn_the_norun = new TH1D(prefix + "hn_the_norun",
                  "Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hn_Emu_norun = new TH1D(prefix + "hn_Emu_norun",
                    "Muon energy; E_{#mu} (GeV); N",
                    100, 0., 150.);

  hn_thmu_norun = new TH1D(prefix + "hn_thmu_norun",
                  "Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hn_x13_norun = new TH1D(prefix + "hn_x13_norun",
                    "x_13 (muon leg); x_{13}; N",
                    100, 0., 1.);

  hn_x24_norun = new TH1D(prefix + "hn_x24_norun",
                    "x_24 (electron leg); x_{24}; N",
                    100, 0., 1.);
  ...
  hsn_t13_norun = new TH1D(prefix + "hsn_t13_norun",
                    "DET t_13 = (pmu_in - pmu_out)^{2}; DET t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_t24_norun = new TH1D(prefix + "hsn_t24_norun",
                    "DET t_24 = (pe_in - pe_out)^{2}; DET t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_Ee_norun = new TH1D(prefix + "hsn_Ee_norun",
                    "DET Electron energy; DET E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hsn_the_norun = new TH1D(prefix + "hsn_the_norun",
                  "DET Electron angle; DET #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsn_Emu_norun = new TH1D(prefix + "hsn_Emu_norun",
                    "DET Muon energy; DET E_{#mu} (GeV); N",
                    100, 0., 150.);

  hsn_thmu_norun = new TH1D(prefix + "hsn_thmu_norun",
                  "DET Muon angle; DET #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hsn_x13_norun = new TH1D(prefix + "hsn_x13_norun",
                    "DET x_13 (muon leg); DET x_{13}; N",
                    100, 0., 1.);

  hsn_x24_norun = new TH1D(prefix + "hsn_x24_norun",
                    "DET x_24 (electron leg); DET x_{24}; N",
                    100, 0., 1.);
  ...
  hn_t13_lep = new TH1D(prefix + "hn_t13_lep",
                    "t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_t24_lep = new TH1D(prefix + "hn_t24_lep",
                    "t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_Ee_lep = new TH1D(prefix + "hn_Ee_lep",
                    "Electron energy; E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hn_the_lep = new TH1D(prefix + "hn_the_lep",
                  "Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hn_Emu_lep = new TH1D(prefix + "hn_Emu_lep",
                    "Muon energy; E_{#mu} (GeV); N",
                    100, 0., 150.);

  hn_thmu_lep = new TH1D(prefix + "hn_thmu_lep",
                  "Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hn_x13_lep = new TH1D(prefix + "hn_x13_lep",
                    "x_13 (muon leg); x_{13}; N",
                    100, 0., 1.);

  hn_x24_lep = new TH1D(prefix + "hn_x24_lep",
                    "x_24 (electron leg); x_{24}; N",
                    100, 0., 1.);
  //
  //////////////////////////////////////////////////////////////////////////////
  ...
  hsn_t13_lep = new TH1D(prefix + "hsn_t13_lep",
                    "DET t_13 = (pmu_in - pmu_out)^{2}; DET t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_t24_lep = new TH1D(prefix + "hsn_t24_lep",
                    "DET t_24 = (pe_in - pe_out)^{2}; DET t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_Ee_lep = new TH1D(prefix + "hsn_Ee_lep",
                    "DET Electron energy; DET E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hsn_the_lep = new TH1D(prefix + "hsn_the_lep",
                  "DET Electron angle; DET #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsn_Emu_lep = new TH1D(prefix + "hsn_Emu_lep",
                    "DET Muon energy; DET E_{#mu} (GeV); N",
                    100, 0., 150.);

  hsn_thmu_lep = new TH1D(prefix + "hsn_thmu_lep",
                  "DET Muon angle; DET #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hsn_x13_lep = new TH1D(prefix + "hsn_x13_lep",
                    "DET x_13 (muon leg); DET x_{13}; N",
                    100, 0., 1.);

  hsn_x24_lep = new TH1D(prefix + "hsn_x24_lep",
                    "DET x_24 (electron leg); DET x_{24}; N",
                    100, 0., 1.);
  ...
  hn_t13_LO = new TH1D(prefix + "hn_t13_LO",
                    "t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_t24_LO = new TH1D(prefix + "hn_t24_LO",
                    "t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hn_Ee_LO = new TH1D(prefix + "hn_Ee_LO",
                    "Electron energy; E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hn_the_LO = new TH1D(prefix + "hn_the_LO",
                  "Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hn_Emu_LO = new TH1D(prefix + "hn_Emu_LO",
                    "Muon energy; E_{#mu} (GeV); N",
                    100, 0., 150.);

  hn_thmu_LO = new TH1D(prefix + "hn_thmu_LO",
                  "Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  if (angleCorrPlots_finebins)
  {
      hn_thmu_vs_the_LO = new TH2D(prefix + "hn_thmu_vs_the_LO",
                        "#theta_{#mu} vs #theta_{e}; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                        1000, 0., 30., 200, 0., 5.);
  } 

  hn_x13_LO = new TH1D(prefix + "hn_x13_LO",
                    "x_13; x_{13}; N",
                    100, 0., 1.);

  hn_x24_LO = new TH1D(prefix + "hn_x24_LO",
                    "x_24; x_{24}; N",
                    100, 0., 1.);
  //
  //////////////////////////////////////////////////////////////////////////////
  ...
  hsn_t13_LO = new TH1D(prefix + "hsn_t13_LO",
                    "DET t_13 = (pmu_in - pmu_out)^{2}; DET t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_t24_LO = new TH1D(prefix + "hsn_t24_LO",
                    "DET t_24 = (pe_in - pe_out)^{2}; DET t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsn_Ee_LO = new TH1D(prefix + "hsn_Ee_LO",
                    "DET Electron energy; DET E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hsn_the_LO = new TH1D(prefix + "hsn_the_LO",
                  "DET Electron angle; DET #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsn_Emu_LO = new TH1D(prefix + "hsn_Emu_LO",
                    "DET Muon energy; DET E_{#mu} (GeV); N",
                    100, 0., 150.);

  hsn_thmu_LO = new TH1D(prefix + "hsn_thmu_LO",
                  "DET Muon angle; DET #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  if (angleCorrPlots_finebins)
  {
      hsn_thmu_vs_the_LO = new TH2D(prefix + "hsn_thmu_vs_the_LO",
                        "DET #theta_{#mu} vs #theta_{e}; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                        1000, 0., 30., 200, 0., 5.);
  }
  hsn_x13_LO = new TH1D(prefix + "hsn_x13_LO",
                    "DET x_13; x_{13}; N",
                    100, 0., 1.);

  hsn_x24_LO = new TH1D(prefix + "hsn_x24_LO",
                    "DET x_24; x_{24}; N",
                    100, 0., 1.);
  ...
  h_resol_thex  = new TH1D(prefix + "h_resol_thex",
                    "Electron #Delta#theta_{X} (E=1GeV); #Delta#theta_{X} (mrad); N",
                    100, -5., +5.);

  h_resol_they  = new TH1D(prefix + "h_resol_they",
                    "Electron #Delta#theta_{Y} (E=1GeV); #Delta#theta_{Y} (mrad); N",
                    100, -5., +5.);

  h_resol_theSpaceXY  = new TH1D(prefix + "h_resol_theSpaceXY",
                    "Electron #Theta_{space} (E=1GeV); #Theta_{space} (mrad); N",
                    100, 0., +5.);

  h_resol_theSpace  = new TH1D(prefix + "h_resol_theSpace",
                    "Electron #Theta_{space} (E=1GeV); #Theta_{space} (mrad); N",
                    100, 0., +5.);

  h_resol_thex5  = new TH1D(prefix + "h_resol_thex5",
                    "Electron #Delta#theta_{X} (E=5GeV); #Delta#theta_{X} (mrad); N",
                    100, -5., +5.);

  h_resol_they5  = new TH1D(prefix + "h_resol_they5",
                    "Electron #Delta#theta_{Y} (E=5GeV); #Delta#theta_{Y} (mrad); N",
                    100, -5., +5.);

  h_resol_theSpace5XY  = new TH1D(prefix + "h_resol_theSpace5XY",
                    "Electron #Theta_{space} (E=5GeV); #Theta_{space} (mrad); N",
                    100, 0., +5.);

  h_resol_theSpace5  = new TH1D(prefix + "h_resol_theSpace5",
                    "Electron #Theta_{space} (E=5GeV); #Theta_{space} (mrad); N",
                    100, 0., +5.);

  h_resol_thmux = new TH1D(prefix + "h_resol_thmux",
                    "Muon #Delta#theta_{X}; #Delta#theta_{X} (mrad); N",
                    100, -1., +1.);

  h_resol_thmuy = new TH1D(prefix + "h_resol_thmuy",
                    "Muon #Delta#theta_{Y}; #Delta#theta_{Y} (mrad); N",
                    100, -1., +1.);

  h_resol_thmuSpaceXY = new TH1D(prefix + "h_resol_thmuSpaceXY",
                    "Muon #Theta; #Theta_{space} (mrad); N",
                    100, 0., +1.);

  //
  h_resol_thexVsE  = new TH2D(prefix + "h_resol_thexVsE",
                    "Electron #Delta#theta_{X} vs Energy; E(GeV); #Delta#theta_{X}",
                    150, 0., 150., 500, -5., +5.);

  h_resol_theyVsE  = new TH2D(prefix + "h_resol_theyVsE",
                    "Electron #Delta#theta_{Y} vs Energy; E(GeV); #Delta#theta_{Y}",
                    150, 0., 150., 500, -5., +5.);

  h_resol_thmuxVsE = new TH2D(prefix + "h_resol_thmuxVsE",
                    "Muon #Delta#theta_{X} vs Energy; E(GeV); #Delta#theta_{X}",
                    150, 0., 150., 500, -5., +5.);

  h_resol_thmuyVsE = new TH2D(prefix + "h_resol_thmuyVsE",
                    "Muon #Delta#theta_{Y} vs Energy; E(GeV); #Delta#theta_{Y}",
                    150, 0., 150., 500, -5., +5.);
  //============================================================================
  //=================== Ratios Smeared / Ideal Variables (with full running) ===
  //============================================================================
  hsratio_t13_Resolution = new TH1D(prefix + "hsratio_t13_Resolution",
                    "t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_t24_Resolution = new TH1D(prefix + "hsratio_t24_Resolution",
                    "t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_the_Resolution = new TH1D(prefix + "hsratio_the_Resolution",
                  "Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsratio_thmu_Resolution = new TH1D(prefix + "hsratio_thmu_Resolution",
                  "Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hsratio_x13_Resolution = new TH1D(prefix + "hsratio_x13_Resolution",
                    "x_13; x_{13}; N",
                    100, 0., 1.);

  hsratio_x24_Resolution = new TH1D(prefix + "hsratio_x24_Resolution",
                    "x_24; x_{24}; N",
                    100, 0., 1.);
  ...
  hsratio_t13_Resolution_LO = new TH1D(prefix + "hsratio_t13_Resolution_LO",
                    "DET t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_t24_Resolution_LO = new TH1D(prefix + "hsratio_t24_Resolution_LO",
                    "DET t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_the_Resolution_LO = new TH1D(prefix + "hsratio_the_Resolution_LO",
                  "DET Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsratio_thmu_Resolution_LO = new TH1D(prefix + "hsratio_thmu_Resolution_LO",
                  "DET Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hsratio_x13_Resolution_LO = new TH1D(prefix + "hsratio_x13_Resolution_LO",
                    "DET x_13; x_{13}; N",
                    100, 0., 1.);

  hsratio_x24_Resolution_LO = new TH1D(prefix + "hsratio_x24_Resolution_LO",
                    "DET x_24; x_{24}; N",
                    100, 0., 1.);
  ...
  gFile->cd("gen/ratios");
  //
  hratio_t13_FullRunning = new TH1D(prefix + "hratio_t13_FullRunning",
                    "t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hratio_t24_FullRunning = new TH1D(prefix + "hratio_t24_FullRunning",
                    "t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hratio_Ee_FullRunning = new TH1D(prefix + "hratio_Ee_FullRunning",
                    "Electron energy; E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hratio_the_FullRunning = new TH1D(prefix + "hratio_the_FullRunning",
                  "Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hratio_Emu_FullRunning = new TH1D(prefix + "hratio_Emu_FullRunning",
                    "Muon energy; E_{#mu} (GeV); N",
                    100, 0., 150.);

  hratio_thmu_FullRunning = new TH1D(prefix + "hratio_thmu_FullRunning",
                  "Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hratio_x13_FullRunning = new TH1D(prefix + "hratio_x13_FullRunning",
                    "x_13 (muon leg); x_{13}; N",
                    100, 0., 1.);

  hratio_x24_FullRunning = new TH1D(prefix + "hratio_x24_FullRunning",
                    "x_24 (electron leg); x_{24}; N",
                    100, 0., 1.);
  ...
  gFile->cd("det/ratios");
  //
  hsratio_t13_FullRunning = new TH1D(prefix + "hsratio_t13_FullRunning",
                    "DET t_13 = (pmu_in - pmu_out)^{2}; DET t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_t24_FullRunning = new TH1D(prefix + "hsratio_t24_FullRunning",
                    "DET t_24 = (pe_in - pe_out)^{2}; DET t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_Ee_FullRunning = new TH1D(prefix + "hsratio_Ee_FullRunning",
                    "DET Electron energy; DET E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hsratio_the_FullRunning = new TH1D(prefix + "hsratio_the_FullRunning",
                  "DET Electron angle; DET #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsratio_Emu_FullRunning = new TH1D(prefix + "hsratio_Emu_FullRunning",
                    "DET Muon energy; DET E_{#mu} (GeV); N",
                    100, 0., 150.);

  hsratio_thmu_FullRunning = new TH1D(prefix + "hsratio_thmu_FullRunning",
                  "DET Muon angle; DET #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hsratio_x13_FullRunning = new TH1D(prefix + "hsratio_x13_FullRunning",
                    "DET x_13 (muon leg); x_{13}; N",
                    100, 0., 1.);

  hsratio_x24_FullRunning = new TH1D(prefix + "hsratio_x24_FullRunning",
                    "DET x_24 (electron leg); x_{24}; N",
                    100, 0., 1.);
  ...
  gFile->cd("gen/ratios");
  //
  hratio_t13_HadronicRunning = new TH1D(prefix + "hratio_t13_HadronicRunning",
                    "t_13 = (pmu_in - pmu_out)^{2}; t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hratio_t24_HadronicRunning = new TH1D(prefix + "hratio_t24_HadronicRunning",
                    "t_24 = (pe_in - pe_out)^{2}; t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hratio_Ee_HadronicRunning = new TH1D(prefix + "hratio_Ee_HadronicRunning",
                    "Electron energy; E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hratio_the_HadronicRunning = new TH1D(prefix + "hratio_the_HadronicRunning",
                  "Electron angle; #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hratio_Emu_HadronicRunning = new TH1D(prefix + "hratio_Emu_HadronicRunning",
                    "Muon energy; E_{#mu} (GeV); N",
                    100, 0., 150.);

  hratio_thmu_HadronicRunning = new TH1D(prefix + "hratio_thmu_HadronicRunning",
                  "Muon angle; #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hratio_x13_HadronicRunning = new TH1D(prefix + "hratio_x13_HadronicRunning",
                    "x_13 (muon leg); x_{13}; N",
                    100, 0., 1.);

  hratio_x24_HadronicRunning = new TH1D(prefix + "hratio_x24_HadronicRunning",
                    "x_24 (electron leg); x_{24}; N",
                    100, 0., 1.);
  ...
  gFile->cd("det/ratios");
  //
  hsratio_t13_HadronicRunning = new TH1D(prefix + "hsratio_t13_HadronicRunning",
                    "DET t_13 = (pmu_in - pmu_out)^{2}; DET t_{13} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_t24_HadronicRunning = new TH1D(prefix + "hsratio_t24_HadronicRunning",
                    "DET t_24 = (pe_in - pe_out)^{2}; DET t_{24} (GeV^{2}); N",
                    100, -0.143, 0.);

  hsratio_Ee_HadronicRunning = new TH1D(prefix + "hsratio_Ee_HadronicRunning",
                    "DET Electron energy; DET E_{e} (GeV); N",
                    100, Emin_e, 140.);

  hsratio_the_HadronicRunning = new TH1D(prefix + "hsratio_the_HadronicRunning",
                  "DET Electron angle; DET #theta_{e} (mrad); N",
                  100, 0., themax); // in mrad

  hsratio_Emu_HadronicRunning = new TH1D(prefix + "hsratio_Emu_HadronicRunning",
                    "DET Muon energy; DET E_{#mu} (GeV); N",
                    100, 0., 150.);

  hsratio_thmu_HadronicRunning = new TH1D(prefix + "hsratio_thmu_HadronicRunning",
                  "DET Muon angle; DET #theta_{#mu} (mrad); N",
                  100, 0., 5.); // in mrad

  hsratio_x13_HadronicRunning = new TH1D(prefix + "hsratio_x13_HadronicRunning",
                    "DET x_13 (muon leg); x_{13}; N",
                    100, 0., 1.);

  hsratio_x24_HadronicRunning = new TH1D(prefix + "hsratio_x24_HadronicRunning",
                    "DET x_24 (electron leg); x_{24}; N",
                    100, 0., 1.);
  ...
  if (doEnergyScale)
  {
    ...
    hsn_thA_ref = new TH1D(prefix + "hsn_thA_ref",
                      "DET Average angle; #theta_{ave} (mrad)",
                      nbins_a, 0., thetaMax_a);

    hsn_thD_vs_thA_ref = new TH2D(prefix + "hsn_thD_vs_thA_ref",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                       nbins_a, 0., thetaMax_a, nbins_a, 0., thetaMax_a);

    hsn_thDs_vs_thA_ref = new TH2D(prefix + "hsn_thDs_vs_thA_ref",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                       nbins_a, 0., thetaMax_a, 2 * nbins_a, -thetaMax_a, thetaMax_a);

    hsn_thA_ref_cut2 = new TH1D(prefix + "hsn_thA_ref_cut2",
                      "DET Average angle; #theta_{ave} (mrad)",
                      nbins_a, 0., thetaMax_a);

    hsn_thD_vs_thA_ref_cut2 = new TH2D(prefix + "hsn_thD_vs_thA_ref_cut2",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                       nbins_a, 0., thetaMax_a, nbins_a, 0., thetaMax_a);

    hsn_thDs_vs_thA_ref_cut2 = new TH2D(prefix + "hsn_thDs_vs_thA_ref_cut2",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                       nbins_a, 0., thetaMax_a, 2 * nbins_a, -thetaMax_a, thetaMax_a);

    gFile->cd("det/escale/LO");
    hsn_thA_LO_ref = new TH1D(prefix + "hsn_thA_LO_ref",
                      "DET Average angle; #theta_{ave} (mrad)",
                      nbins_a, 0., thetaMax_a);

    hsn_thD_vs_thA_LO_ref = new TH2D(prefix + "hsn_thD_vs_thA_LO_ref",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                       nbins_a, 0., thetaMax_a, nbins_a, 0., thetaMax_a);

    hsn_thDs_vs_thA_LO_ref = new TH2D(prefix + "hsn_thDs_vs_thA_LO_ref",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                      nbins_a, 0., thetaMax_a, 2 * nbins_a, -thetaMax_a, thetaMax_a);

    hsn_thA_LO_ref_cut2 = new TH1D(prefix + "hsn_thA_LO_ref_cut2",
                      "DET Average angle; #theta_{ave} (mrad)",
                      nbins_a, 0., thetaMax_a);

    hsn_thD_vs_thA_LO_ref_cut2 = new TH2D(prefix + "hsn_thD_vs_thA_LO_ref_cut2",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                       nbins_a, 0., thetaMax_a, nbins_a, 0., thetaMax_a);

    hsn_thDs_vs_thA_LO_ref_cut2 = new TH2D(prefix + "hsn_thDs_vs_thA_LO_ref_cut2",
                      "DET Diff angle vs Average angle; #theta_{ave} (mrad); #theta_{dif} (mrad)",
                       nbins_a, 0., thetaMax_a, 2 * nbins_a, -thetaMax_a, thetaMax_a);
    ...
    cout << "\n theta histos for E scale calibration have bins of "
         << thbin << " mrad" << endl;
    cout <<  " max theta (electron, Right, Left) = " << thetaMax << " mrad,"
         << " (muon) = " << thmumax << " mrad, (average) = " << thavmax << " mrad" << endl;
    cout <<  " N bins    (                     ) = " << nbmax    << "       (    ) = "
         << nbmumax << "       (       ) = " << nbavmax << endl;

    // NLO
    gFile->cd("det/escale/NLO");

    hesn_thmu_vs_the = new TH2D(prefix + "hesn_thmu_vs_the",
                      "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                      nbmax, 0., thmax, nbmumax, 0., thmumax);

    hesn_thL_vs_thR = new TH2D(prefix + "hesn_thL_vs_thR",
                      "DET L vs R angles; #theta_{R} (mrad); #theta_{L} (mrad)",
                      nbmax, 0., thmax, nbmax, 0., thmax);

    hesn_thD_vs_thA = new TH2D(prefix + "hesn_thD_vs_thA",
                      "DET |#theta_{dif}| vs #theta_{ave}; #theta_{ave} (mrad); |#theta_{dif}| (mrad)",
                      nbavmax, 0., thavmax, nbmax, 0., thmax);

    hesn_thD_vs_alpha = new TH2D(prefix + "hesn_thD_vs_alpha",
                      "DET |#theta_{dif}| vs Opening angle; #alpha (mrad); |#theta_{dif}| (mrad)",
                      nbmax, 0., thmax, nbmax, 0., thmax);

    hesn_alpha_vs_thA = new TH2D(prefix + "hesn_alpha_vs_thA",
                      "DET Opening Angle vs #theta_{ave}; #theta_{ave} (mrad); #alpha (mrad)",
                      nbavmax, 0., thavmax, nbmax, 0., thmax);
    // LO
    gFile->cd("det/escale/LO");

    hesn_thmu_vs_the_LO = new TH2D(prefix + "hesn_thmu_vs_the_LO",
                      "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                      nbmax, 0., thmax, nbmumax, 0., thmumax);

    hesn_thL_vs_thR_LO = new TH2D(prefix + "hesn_thL_vs_thR_LO",
                      "DET L vs R angles; #theta_{R} (mrad); #theta_{L} (mrad)",
                      nbmax, 0., thmax, nbmax, 0., thmax);

    hesn_thD_vs_thA_LO = new TH2D(prefix + "hesn_thD_vs_thA_LO",
                      "DET |#theta_{dif}| vs #theta_{ave}; #theta_{ave} (mrad); |#theta_{dif}| (mrad)",
                      nbavmax, 0., thavmax, nbmax, 0., thmax);

    hesn_thD_vs_alpha_LO = new TH2D(prefix + "hesn_thD_vs_alpha_LO",
                      "DET |#theta_{dif}| vs Opening angle; #alpha (mrad); |#theta_{dif}| (mrad)",
                      nbmax, 0., thmax, nbmax, 0., thmax);

    hesn_alpha_vs_thA_LO = new TH2D(prefix + "hesn_alpha_vs_thA_LO",
                      "DET Opening Angle vs #theta_{ave}; #theta_{ave} (mrad); #alpha (mrad)",
                      nbavmax, 0., thavmax, nbmax, 0., thmax);
  ...
  hn_thmuVsthe_NLO_ref = new TH2D(prefix + "hn_thmuVsthe_NLO_ref",
                    "GEN Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                    nbins, thetat, nbins_s, thetam);

  hn_thL_vs_thR_NLO_ref = new TH2D(prefix + "hn_thL_vs_thR_NLO_ref",
                    "GEN L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                    nbins, thetat, nbins, thetat);

  // only-leptonic running
  gFile->cd("gen/templ/NLO/lep");

  hn_thmuVsthe_NLO_lep = new TH2D(prefix + "hn_thmuVsthe_NLO_lep",
                    "GEN Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                    nbins, thetat, nbins_s, thetam);

  hn_thL_vs_thR_NLO_lep = new TH2D(prefix + "hn_thL_vs_thR_NLO_lep",
                    "GEN L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                    nbins, thetat, nbins, thetat);
  ...
  if (iLumi == 0)
  {
    for (unsigned int iK = 0; iK < ngrid; ++iK)
    {
      for (unsigned int iM = 0; iM < ngrid; ++iM)
      {
        ...
        hn_thmuVsthe_NLO_templ.push_back(new TH2D(prefix + "hn_thmuVsthe_NLO_templ" + name,
                                                  "GEN Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                                  nbins, thetat, nbins_s, thetam));

        hn_thL_vs_thR_NLO_templ.push_back(new TH2D(prefix + "hn_thL_vs_thR_NLO_templ" + name,
                                                  "GEN L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                                  nbins, thetat, nbins, thetat));
      }
    }
  ...
  hsn_thmuVsthe_NLO_ref = new TH2D(prefix + "hsn_thmuVsthe_NLO_ref",
                                  "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                  nbins, thetat, nbins_s, thetam);
  hsn_thL_vs_thR_NLO_ref = new TH2D(prefix + "hsn_thL_vs_thR_NLO_ref",
                                  "DET L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                  nbins, thetat, nbins, thetat);
  // only-leptonic running
  gFile->cd("det/templ/NLO/lep");
  hsn_thmuVsthe_NLO_lep = new TH2D(prefix + "hsn_thmuVsthe_NLO_lep",
                                  "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                  nbins, thetat, nbins_s, thetam);
  hsn_thL_vs_thR_NLO_lep = new TH2D(prefix + "hsn_thL_vs_thR_NLO_lep",
                                  "DET L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                  nbins, thetat, nbins, thetat);
  ...
  if (iLumi == 0)
  {
    for (unsigned int iK = 0; iK < ngrid; ++iK)
    {
      for (unsigned int iM = 0; iM < ngrid; ++iM)
      {
        char name[100];
        sprintf(name, "_K%i_M%i", iK, iM);
        hsn_thmuVsthe_NLO_templ.push_back(new TH2D(prefix + "hsn_thmuVsthe_NLO_templ" + name,
                                                  "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                                  nbins, thetat, nbins_s, thetam));
        hsn_thL_vs_thR_NLO_templ.push_back(new TH2D(prefix + "hsn_thL_vs_thR_NLO_templ" + name,
                                                  "DET L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                                  nbins, thetat, nbins, thetat));
      }
    }
  ...
  hn_thmuVsthe_LO_ref = new TH2D(prefix + "hn_thmuVsthe_LO_ref",
                                "GEN Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                nbins, thetat, nbins_s, thetam);

  hn_thL_vs_thR_LO_ref = new TH2D(prefix + "hn_thL_vs_thR_LO_ref",
                                "GEN L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                nbins, thetat, nbins, thetat);
  // only-leptonic running
  gFile->cd("gen/templ/LO/lep");

  hn_thmuVsthe_LO_lep = new TH2D(prefix + "hn_thmuVsthe_LO_lep",
                                "GEN Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                nbins, thetat, nbins_s, thetam);

  hn_thL_vs_thR_LO_lep = new TH2D(prefix + "hn_thL_vs_thR_LO_lep",
                                "GEN L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                nbins, thetat, nbins, thetat);
  ...
  if (iLumi == 0)
  {
    for (unsigned int iK = 0; iK < ngrid; ++iK)
    {
      for (unsigned int iM = 0; iM < ngrid; ++iM)
      {
        char name[100];
        sprintf(name, "_K%i_M%i", iK, iM);
        hn_thmuVsthe_LO_templ.push_back(new TH2D(prefix + "hn_thmuVsthe_LO_templ" + name,
                                                "GEN Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                                nbins, thetat, nbins_s, thetam));
        hn_thL_vs_thR_LO_templ.push_back(new TH2D(prefix + "hn_thL_vs_thR_LO_templ" + name,
                                                "GEN L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                                nbins, thetat, nbins, thetat));
      }
    }
  ...
  hsn_thmuVsthe_LO_ref = new TH2D(prefix + "hsn_thmuVsthe_LO_ref",
                                "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                nbins, thetat, nbins_s, thetam);

  hsn_thL_vs_thR_LO_ref = new TH2D(prefix + "hsn_thL_vs_thR_LO_ref",
                                "DET L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                nbins, thetat, nbins, thetat);
  // only-leptonic running
  gFile->cd("det/templ/LO/lep");
  hsn_thmuVsthe_LO_lep = new TH2D(prefix + "hsn_thmuVsthe_LO_lep",
                                "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                nbins, thetat, nbins_s, thetam);

  hsn_thL_vs_thR_LO_lep = new TH2D(prefix + "hsn_thL_vs_thR_LO_lep",
                                "DET L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                nbins, thetat, nbins, thetat);
  ...
  if (iLumi == 0)
  {
    for (unsigned int iK = 0; iK < ngrid; ++iK)
    {
      for (unsigned int iM = 0; iM < ngrid; ++iM)
      {
        char name[100];
        sprintf(name, "_K%i_M%i", iK, iM);
        hsn_thmuVsthe_LO_templ.push_back(new TH2D(prefix + "hsn_thmuVsthe_LO_templ" + name,
                                                  "DET Electron and Muon angles; #theta_{e} (mrad); #theta_{#mu} (mrad)",
                                                  nbins, thetat, nbins_s, thetam));

        hsn_thL_vs_thR_LO_templ.push_back(new TH2D(prefix + "hsn_thL_vs_thR_LO_templ" + name,
                                                  "DET L vs R angles; #theta_{L} (mrad); #theta_{R} (mrad)",
                                                  nbins, thetat, nbins, thetat));
      }
    }
    ...
    for (unsigned int iK = 0; iK < ngrid; ++iK)
    {
      for (unsigned int iM = 0; iM < ngrid; ++iM)
      {
        ...
        double wgtRew = reweight(iparam, par, npart, event.coef,
                                 invalfa0, mass_e, mass_mu, t24, t24k1, t24k2,
                                 t24k1k2);
...
void Histos::Fill(const MuE::Event & event, const MuE::MuEana & a)
{
  ...
  hsn_coo_mu->Fill(a.detKin.cooxmu, a.detKin.cooymu, a.wgt_full);
  hsn_coo_e->Fill(a.detKin.cooxe, a.detKin.cooye, a.wgt_full);
  hsn_thmu_vs_the_defAngles->Fill(a.detKin.def_angle_e, a.detKin.def_angle_mu, a.wgt_full);
  //
  ...
  // radiated photon histos (DET level)
  if (a.photon1.energy > 0)
  {
    hn_egamma->Fill(a.photon1.energy, a.wgt_full);
    hn_egamma_CoM->Fill(a.photon1.energyCoM, a.wgt_full);

    if (a.photon1.energy > 0.75)
    {
      hn_thgamma->Fill(a.photon1.theta, a.wgt_full);
    }
  }
...
void Histos::Plot2D(bool debug)
{
  ...
  TCanvas cz_thmuVsthe("cz_thmuVsthe",
                       "Muon vs Electron angle", 700, 500);
  hsn_thmuVsthe_NLO_ref->Draw("COLZ");
  cz_thmuVsthe.SaveAs(pname + "thmuVsthe_det_colz.png");

  TCanvas cz_thmuVsthe_defAngles("cz_thmuVsthe_defAngles",
                                 "Muon vs Electron angle", 700, 500);
  hsn_thmu_vs_the_defAngles->Draw("COLZ");
  cz_thmuVsthe.SaveAs(pname + "thmu_vs_the_defAngles.png");

  TCanvas cz_thL_vs_thR("cz_thL_vs_thR",
                        "Theta_L vs Theta_R angle", 700, 700);
...
void Histos::RatioFinal(Long_t n_events, TFile *fp, TString hname)
{
  ...
  cout << "\n*** Computing Final ratios (with Projected statistics) for histogram: "
       << hname << endl;
  ...
  cerr << "\n" << "Loading Reference root histograms from the existing file: "
       << fp->GetName() << endl;
  ...
  // ... add und/ovf
  cout << "underflow content and error: " << hn->GetBinContent(0) << " +/- "
       << hn->GetBinError(0) << endl;

  cout << "overflow  content and error: " << hn->GetBinContent(nbinsm + 1)
       << " +/- " << hn->GetBinError(nbinsm + 1) << endl;
  ...
  for (Int_t im = 0; im <= nbinsm + 1; ++im)
  {
    ...
    cout << "bin " << im << ", GEN error: " << relerr_gen << ", PROJ error: "
         << relerr_proj << ", stat.factor = " << fstat << endl;
  }
  ...
  if (hratioFinal_HadronicRunning->GetNbinsX() != nbinsm)
    {cout << "***ERROR*** 2" << endl; exit(2);}
  for (Int_t im = 0; im <= nbinsm + 1; ++im)
  {
  ...
  for (Int_t im = 0; im <= nbinsm + 1; ++im)
  {
    ...
    cout << "bin " << im << " (" << x_im << ")" << ", value: " << val_im
         << ", error: " << err_im << ", RelErr: " << relerr_im << endl;
  }
}
  ...
  for (const auto & kdist : kine_distributions)
  {
    ...
    for (const auto & idir : mc_inputs)
    {
      ...
      while (getline(carlos_file, line))
      {
        if (ibin >= 100)
        {
          cerr << endl << "Error: more than 100 lines in input Carlos file: "
               << fname << endl;
          exit(300);
        }
      ...
      if (ibin < 100)
      {
        cerr << endl << "Error: less than 100 lines in input Carlos file: "
             << fname << endl;
        exit(400);
      }
    }
...
void Histos::CompareWithCarlos(bool debug)
{
  ...
  for (const auto & kdist : kine_distributions)
  {
    ...
    TGraph g_gio(100), g_gio_err(100),
           g_carlo(100),g_carlo_err(100),
           g_diff(100), g_diff_err(100);
    ...
    while (getline(carlos_file, line))
    {
      ...
      if (debug)
        cout << "c_content, content = " << c_content << " " << content
             << " rel diff cont = " << diff << "\t c_error, error     = " <<
             c_error << " " << error << " rel diff error = " << diff_err  << endl;
    ...
    if (debug)
      cout << "*** MAX Relative Difference in contents : " <<  maxdif
           << ", at bin : " << ibmax
           << "*** MAX Relative Difference in errors : " <<  maxdif_err
           << ", at bin : " << ibmaxe << endl;
